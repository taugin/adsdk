...
public class RewardManager implements Handler.Callback {

    private static RewardManager sRewardManager;

    public static RewardManager get(Context context) {
    ...
    private static void createInstance(Context context) {
        synchronized (RewardManager.class) {
            if (sRewardManager == null) {
                sRewardManager = new RewardManager(context);
            }
        }
    }
    ...
    private RewardManager(Context context) {
        ...
        mAllAdPlaces.add("reward_place2");
        mAllAdPlaces.add("reward_place3");
        mAllAdPlaces.add("reward_place4");
        mContext = context.getApplicationContext();
        mHandler = new Handler(this);
        mListeners = new Stack<OnRewardListener>();
    ...
    private boolean needRemove(RewardItem item) {
        ...
        return !(AdSdk.get(mContext).isInterstitialLoaded(item.pidName) || item.showing);
    }
    ...
    private boolean isShowTimeout(RewardItem item) {
        ...
        return item.showing && System.currentTimeMillis() - item.time > 60 * 1000;
    }
    ...
    public void registerListener(OnRewardListener l) {
        if (mListeners != null) {
            mListeners.push(l);
        }
        sendMessageDelayInternal(false);
    ...
    public void unregisterListener(OnRewardListener l) {
        if (mListeners != null) {
            mListeners.pop();
        }
